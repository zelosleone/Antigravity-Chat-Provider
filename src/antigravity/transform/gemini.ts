...
function normalizeGeminiTools(payload: RequestPayload): void {
  ...
  payload.tools = (payload.tools as unknown[]).map((tool: unknown) => {
    ...
    if (!newTool.function && !newTool.parameters && !newTool.input_schema && !newTool.inputSchema) {
...
export function applyGeminiTransforms(
  ...
  if (normalizedThinking) {
    const thinkingConfig = tierThinkingLevel && isGemini3Model(model)
      ? buildGemini3ThinkingConfig(normalizedThinking.includeThoughts ?? true, tierThinkingLevel)
