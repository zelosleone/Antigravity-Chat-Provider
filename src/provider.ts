...
async function canUseStoredAuth(
  context: vscode.ExtensionContext,
): Promise<boolean> {
  return ensureValidAuth(context, false).then(
    () => true,
    () => false,
  );
}
...
export class AntigravityChatProvider
  implements vscode.LanguageModelChatProvider
{
  ...
  async provideLanguageModelChatResponse(
    ...
    const projectId =
      getProjectIdFromAuth(auth) ?? ANTIGRAVITY_DEFAULT_PROJECT_ID;
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          text || `Antigravity request failed (${response.status})`,
        );
      }
